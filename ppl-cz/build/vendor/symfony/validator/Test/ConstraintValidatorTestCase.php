 ...
 */
namespace PPLCZVendor\Symfony\Component\Validator\Test;

use PPLCZVendor\PHPUnit\Framework\Assert;
use PPLCZVendor\PHPUnit\Framework\Constraint\IsIdentical;
use PPLCZVendor\PHPUnit\Framework\Constraint\IsInstanceOf;
use PPLCZVendor\PHPUnit\Framework\Constraint\IsNull;
use PPLCZVendor\PHPUnit\Framework\Constraint\LogicalOr;
use PPLCZVendor\PHPUnit\Framework\ExpectationFailedException;
use PPLCZVendor\PHPUnit\Framework\TestCase;
use PPLCZVendor\Symfony\Component\Validator\Constraint;
use PPLCZVendor\Symfony\Component\Validator\Constraints\NotNull;
use PPLCZVendor\Symfony\Component\Validator\Constraints\Valid;
use PPLCZVendor\Symfony\Component\Validator\ConstraintValidatorInterface;
use PPLCZVendor\Symfony\Component\Validator\ConstraintViolation;
use PPLCZVendor\Symfony\Component\Validator\ConstraintViolationInterface;
use PPLCZVendor\Symfony\Component\Validator\ConstraintViolationList;
use PPLCZVendor\Symfony\Component\Validator\ConstraintViolationListInterface;
use PPLCZVendor\Symfony\Component\Validator\Context\ExecutionContext;
use PPLCZVendor\Symfony\Component\Validator\Context\ExecutionContextInterface;
use PPLCZVendor\Symfony\Component\Validator\Mapping\ClassMetadata;
use PPLCZVendor\Symfony\Component\Validator\Mapping\PropertyMetadata;
use PPLCZVendor\Symfony\Component\Validator\Validator\ContextualValidatorInterface;
...
abstract class ConstraintValidatorTestCase extends TestCase
{
    ...
    protected function createContext()
    {
        ...
        $validator->expects($this->any())->method('validate')->willReturnCallback(function () {
        ...
        $contextualValidatorMockBuilder = $this->getMockBuilder(AssertingContextualValidator::class)->setConstructorArgs([$context]);
        $contextualValidatorMethods = ['atPath', 'validate', 'validateProperty', 'validatePropertyValue', 'getViolations'];
        ...
        $contextualValidator->expects($this->any())->method('atPath')->willReturnCallback(function ($path) use($contextualValidator) {
            return $contextualValidator->doAtPath($path);
        });
        $contextualValidator->expects($this->any())->method('validate')->willReturnCallback(function ($value, $constraints = null, $groups = null) use($contextualValidator) {
            return $contextualValidator->doValidate($value, $constraints, $groups);
        });
        $contextualValidator->expects($this->any())->method('validateProperty')->willReturnCallback(function ($object, $propertyName, $groups = null) use($contextualValidator) {
            return $contextualValidator->validateProperty($object, $propertyName, $groups);
        });
        $contextualValidator->expects($this->any())->method('validatePropertyValue')->willReturnCallback(function ($objectOrClass, $propertyName, $value, $groups = null) use($contextualValidator) {
            return $contextualValidator->doValidatePropertyValue($objectOrClass, $propertyName, $value, $groups);
        });
        $contextualValidator->expects($this->any())->method('getViolations')->willReturnCallback(function () use($contextualValidator) {
            return $contextualValidator->doGetViolations();
        });
        $validator->expects($this->any())->method('inContext')->with($context)->willReturn($contextualValidator);
        return $context;
    }
...
final class ConstraintViolationAssertion
{
    ...
    {
        return new ConstraintViolation($this->message, $this->message, $this->parameters, $this->context->getRoot(), $this->propertyPath, $this->invalidValue, $this->plural, $this->code, $this->constraint, $this->cause);
    }
}
/**
 * @internal
