...
class Dumper
{
    public const DEPTH = 'depth', TRUNCATE = 'truncate', COLLAPSE = 'collapse', COLLAPSE_COUNT = 'collapsecount', LOCATION = 'location', OBJECT_EXPORTERS = 'exporters', LAZY = 'lazy', LIVE = 'live', SNAPSHOT = 'snapshot', DEBUGINFO = 'debuginfo', KEYS_TO_HIDE = 'keystohide';
    // sensitive keys not displayed (defaults to [])
    public const LOCATION_SOURCE = 0b1, LOCATION_LINK = 0b10, LOCATION_CLASS = 0b100;
    // shows where class is defined
    public const HIDDEN_VALUE = '*****';
    /** @var array */
    public static $liveSnapshot = [];
    /** @var array */
    public static $terminalColors = ['bool' => '1;33', 'null' => '1;33', 'number' => '1;32', 'string' => '1;36', 'array' => '1;31', 'key' => '1;37', 'object' => '1;31', 'visibility' => '1;30', 'resource' => '1;37', 'indent' => '1;30'];
    /** @var array */
    public static $resources = ['stream' => 'stream_get_meta_data', 'stream-context' => 'stream_context_get_options', 'curl' => 'curl_getinfo'];
    /** @var array */
    public static $objectExporters = ['Closure' => [self::class, 'exportClosure'], 'SplFileInfo' => [self::class, 'exportSplFileInfo'], 'SplObjectStorage' => [self::class, 'exportSplObjectStorage'], '__PHP_Incomplete_Class' => [self::class, 'exportPhpIncompleteClass']];
    /** @var int|null */
    private $maxDepth = 4;
    /** @var int|null */
    ...
    private function __construct(array $options = [])
    {
        ...
        $this->location = $this->location === \true ? ~0 : (int) $this->location;
        $this->snapshot =& $options[self::SNAPSHOT];
        if ($options[self::LIVE] ?? \false) {
            $this->snapshot =& self::$liveSnapshot;
        }
        $this->lazy = \is_array($this->snapshot) ? \true : $options[self::LAZY] ?? $this->lazy;
    ...
    private function asHtml($var) : string
    {
        ...
        $locAttrs = $file && $this->location & self::LOCATION_SOURCE ? Helpers::formatHtml(' title="%in file % on line %" data-tracy-href="%"', "{$code}\n", $file, $line, Helpers::editorUri($file, $line)) : null;
        if (\is_array($this->snapshot)) {
            $options[self::SNAPSHOT] =& $this->snapshot;
        }
        $snapshot =& $options[self::SNAPSHOT];
        ...
        return '<pre class="tracy-dump' . ($json && $this->collapseTop === \true ? ' tracy-collapsed' : '') . '"' . $locAttrs . (\is_array($snapshot) && !\is_array($this->snapshot) ? ' data-tracy-snapshot=' . $this->formatSnapshotAttribute($snapshot) : '') . ($json ? " data-tracy-dump='" . \json_encode($json, \JSON_HEX_APOS | \JSON_HEX_AMP) . "'>" : '>') . $html . ($file && $this->location & self::LOCATION_LINK ? '<small>in ' . Helpers::editorLink($file, $line) . '</small>' : '') . "</pre>\n";
    }
    /**
     * Dumps variable to x-terminal.
     */
    private function asTerminal($var, array $colors = []) : string
    {
        ...
        if ($colors) {
            $s = \preg_replace_callback('#<span class="tracy-dump-(\\w+)">|</span>#', function ($m) use($colors) : string {
                return "\x1b[" . (isset($m[1], $colors[$m[1]]) ? $colors[$m[1]] : '0') . 'm';
            }, $s);
        }
    ...
    private function dumpVar(&$var, array $options, int $level = 0) : string
    {
        if (!\method_exists(self::class, $m = 'dump' . \explode(' ', \gettype($var))[0])) {
            $m = 'dumpResource';
            // closed resource is 'unknown type' in PHP 7.1
        }
    ...
    {
        $var = \is_finite($var) ? ($tmp = \json_encode($var)) . (\strpos($tmp, '.') === \false ? '.0' : '') : \var_export($var, \true);
        return "<span class=\"tracy-dump-number\">{$var}</span>\n";
    }
    private function dumpString(&$var) : string
    {
        return '<span class="tracy-dump-string">"' . Helpers::escapeHtml($this->encodeString($var, $this->maxLength)) . '"</span>' . (\strlen($var) > 1 ? ' (' . \strlen($var) . ')' : '') . "\n";
    }
    private function dumpArray(&$var, array $options, int $level) : string
    {
        ...
        if (empty($var)) {
            ...
            $collapsed = $level ? \count($var) >= $this->collapseSub : (\is_int($this->collapseTop) ? \count($var) >= $this->collapseTop : $this->collapseTop);
            ...
            if ($collapsed && $options[self::LAZY] !== \false) {
                ...
                return $span . " data-tracy-dump='" . \json_encode($this->toJson($var, $options, $level), \JSON_HEX_APOS | \JSON_HEX_AMP) . "'>" . $out . \count($var) . ")</span>\n";
            } else {
                ...
                try {
                    ...
                    foreach ($var as $k => &$v) {
                        ...
                        $out .= '<span class="tracy-dump-indent">   ' . \str_repeat('|  ', $level) . '</span>' . '<span class="tracy-dump-key">' . Helpers::escapeHtml($this->encodeKey($k)) . '</span> => ' . ($hide ? Helpers::escapeHtml(self::hideValue($v)) . "\n" : $this->dumpVar($v, $options, $level + 1));
                    }
                } finally {
                    unset($var[$marker]);
                }
    ...
    private function dumpObject(&$var, array $options, int $level) : string
    {
        ...
        if ($this->location & self::LOCATION_CLASS) {
            ...
            if ($editor) {
                $editorAttributes = Helpers::formatHtml(' title="Declared in file % on line %" data-tracy-href="%"', $rc->getFileName(), $rc->getStartLine(), $editor);
            }
        }
        $out = '<span class="tracy-dump-object"' . $editorAttributes . '>' . Helpers::escapeHtml(Helpers::getClass($var)) . '</span> <span class="tracy-dump-hash">#' . \substr(\md5(\spl_object_hash($var)), 0, 4) . '</span>';
        if (empty($fields)) {
            return $out . "\n";
        } elseif (\in_array($var, $options['parents'] ?? [], \true)) {
            return $out . " { <i>RECURSION</i> }\n";
        } elseif (!$this->maxDepth || $level < $this->maxDepth || $var instanceof \Closure) {
            $collapsed = $level ? \count($fields) >= $this->collapseSub : (\is_int($this->collapseTop) ? \count($fields) >= $this->collapseTop : $this->collapseTop);
            $span = '<span class="tracy-toggle' . ($collapsed ? ' tracy-collapsed' : '') . '"';
            if ($collapsed && $options[self::LAZY] !== \false) {
                return $span . " data-tracy-dump='" . \json_encode($this->toJson($var, $options, $level), \JSON_HEX_APOS | \JSON_HEX_AMP) . "'>" . $out . "</span>\n";
            } else {
                $out = $span . '>' . $out . "</span>\n" . '<div' . ($collapsed ? ' class="tracy-collapsed"' : '') . '>';
                $options['parents'][] = $var;
                foreach ($fields as $k => &$v) {
                    ...
                    $hide = \is_string($k) && isset($this->keysToHide[\strtolower($k)]);
                    $out .= '<span class="tracy-dump-indent">   ' . \str_repeat('|  ', $level) . '</span>' . '<span class="tracy-dump-key">' . Helpers::escapeHtml($this->encodeKey($k)) . "</span>{$vis} => " . ($hide ? Helpers::escapeHtml(self::hideValue($v)) . "\n" : $this->dumpVar($v, $options, $level + 1));
                }
    ...
    private function dumpResource(&$var, array $options, int $level) : string
    {
        ...
        $out = '<span class="tracy-dump-resource">' . Helpers::escapeHtml($type) . ' resource</span> ' . '<span class="tracy-dump-hash">#' . (int) $var . '</span>';
        if (isset($this->resourceDumpers[$type])) {
            $out = "<span class=\"tracy-toggle tracy-collapsed\">{$out}</span>\n<div class=\"tracy-collapsed\">";
            foreach ($this->resourceDumpers[$type]($var) as $k => $v) {
                $out .= '<span class="tracy-dump-indent">   ' . \str_repeat('|  ', $level) . '</span>' . '<span class="tracy-dump-key">' . Helpers::escapeHtml($k) . '</span> => ' . $this->dumpVar($v, $options, $level + 1);
            }
            return $out . '</div>';
        }
        return "{$out}\n";
    ...
    private function toJson(&$var, array $options = [], int $level = 0)
    {
        if (\is_bool($var) || $var === null || \is_int($var)) {
            ...
            return \is_finite($var) ? \strpos($tmp = \json_encode($var), '.') ? $var : ['number' => "{$tmp}.0"] : ['type' => (string) $var];
            ...
            $obj = $obj ?: ['id' => \count($options[self::SNAPSHOT]), 'name' => Helpers::getClass($var), 'hash' => \substr(\md5($hash), 0, 4), 'level' => $level, 'object' => $var];
    ...
    {
        return \is_int($key) || \preg_match('#^[!\\#$%&()*+,./0-9:;<=>?@A-Z[\\]^_`a-z{|}~-]{1,50}$#D', $key) && !\preg_match('#^true|false|null$#iD', $key) ? $key : '"' . $this->encodeString($key, $this->maxLength) . '"';
    }
    /**
     * @param  object  $obj
     */
    ...
    private static function exportClosure(\Closure $obj) : array
    {
        ...
        return ['file' => $rc->getFileName(), 'line' => $rc->getStartLine(), 'variables' => $rc->getStaticVariables(), 'parameters' => \implode(', ', $res)];
    }
    ...
    {
        return self::$terminalColors && (\getenv('ConEmuANSI') === 'ON' || \getenv('ANSICON') !== \false || \getenv('term') === 'xterm-256color' || \defined('STDOUT') && \function_exists('posix_isatty') && \posix_isatty(\STDOUT));
    }
}
