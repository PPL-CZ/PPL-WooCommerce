...
class BlueScreen
{
    ...
    {
        $this->collapsePaths[] = \preg_match('#(.+/vendor)/tracy/tracy/src/Tracy/BlueScreen$#', \strtr(__DIR__, '\\', '/'), $m) ? $m[1] : __DIR__;
    }
    /**
    ...
    private function renderTemplate(\Throwable $exception, string $template, $toScreen = \true) : void
    {
        ...
        $snapshot = [];
        $dump = $this->getDumper();
        $css = \array_map('file_get_contents', \array_merge([__DIR__ . '/assets/bluescreen.css', __DIR__ . '/../Toggle/toggle.css', __DIR__ . '/../TableSort/table-sort.css', __DIR__ . '/../Dumper/assets/dumper.css'], Debugger::$customCssFiles));
        ...
        if ($vars) {
            $out = \preg_replace_callback('#">\\$(\\w+)(&nbsp;)?</span>#', function (array $m) use($vars, $keysToHide) : string {
                if (\array_key_exists($m[1], $vars)) {
                    $dump = Dumper::toHtml($vars[$m[1]], [Dumper::DEPTH => 1, Dumper::KEYS_TO_HIDE => $keysToHide]);
                    return '" title="' . \str_replace('"', '&quot;', \trim(\strip_tags($dump))) . $m[0];
                }
                return $m[0];
            }, $out);
        }
    ...
    public function getDumper() : \Closure
    {
        ...
        return function ($v, $k = null) use($keysToHide) : string {
            ...
            return Dumper::toHtml($v, [Dumper::DEPTH => $this->maxDepth, Dumper::TRUNCATE => $this->maxLength, Dumper::SNAPSHOT => &$this->snapshot, Dumper::LOCATION => Dumper::LOCATION_CLASS, Dumper::KEYS_TO_HIDE => $this->keysToHide]);
        };
    }
    private function formatMessage(\Throwable $exception) : string
    {
        ...
        $msg = \preg_replace_callback('#([\\w\\\\/.:-]+\\.(?:php|phpt|phtml|latte|neon))(?|:(\\d+)| on line (\\d+))?#', function ($m) {
            return @\is_file($m[1]) ? '<a href="' . Helpers::escapeHtml(Helpers::editorUri($m[1], isset($m[2]) ? (int) $m[2] : null)) . '">' . $m[0] . '</a>' : $m[0];
        }, $msg);
        return $msg;
    }
