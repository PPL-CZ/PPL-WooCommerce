...
class Helpers
{
    ...
    public static function editorLink(string $file, int $line = null) : string
    {
        ...
        if ($editor = self::editorUri($origFile, $line)) {
            ...
            return self::formatHtml('<a href="%" title="%">%<b>%</b>%</a>', $editor, $origFile . ($line ? ":{$line}" : ''), \rtrim(\dirname($file), \DIRECTORY_SEPARATOR) . \DIRECTORY_SEPARATOR, \basename($file), $line ? ":{$line}" : '');
        } else {
    ...
    public static function improveException(\Throwable $e) : void
    {
        ...
        if (!$e instanceof \Error && !$e instanceof \ErrorException || $e instanceof \PPLCZVendor\Nette\MemberAccessException || \strpos($e->getMessage(), 'did you mean')) {
            // do nothing
