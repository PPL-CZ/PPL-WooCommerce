    ...
    public static function dump($var, bool $return = \false)
    {
        if ($return) {
            return Helpers::capture(function () use($var) {
                Dumper::dump($var, [Dumper::DEPTH => self::$maxDepth, Dumper::TRUNCATE => self::$maxLength]);
            });
        } elseif (!self::$productionMode) {
            Dumper::dump($var, [Dumper::DEPTH => self::$maxDepth, Dumper::TRUNCATE => self::$maxLength, Dumper::LOCATION => self::$showLocation]);
        }
        return $var;
    }
    /**
